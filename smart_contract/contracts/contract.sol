...
contract OrganizationNFT is ERC721, Ownable {
    ...
    function getOrganizationDetails(uint256 _organizationId)
        ...
        returns (
            ...
            string[] memory expenditureCategoryNames,
            uint256[] memory expenditureCategorybalance,
            string memory chain_address
        )
    {
        Organization storage org = organizations[_organizationId];
        ...
        uint numExpenditureCat = org.cat_expenditure_count;
        expenditureCategoryNames = new string[](numExpenditureCat);
        expenditureCategorybalance = new uint256[](numExpenditureCat);

        for (uint256 i = 0; i < numExpenditureCat; i++) {
            expenditureCategoryNames[i] = org.expenditure_categories[i].name;
            expenditureCategorybalance[i] = org.expenditure_categories[i].balance;
        }

        return (org.name, org.description, org.organizationAddress, org.funds, categoryNames, categorybalance, expenditureCategoryNames, expenditureCategorybalance, org.chain_address);
    }

    // Function to get organization details by wallet address
    function getOrganizationsByAddress()
        external
        view
        returns (
            uint256 org_id
        ...
        for (uint256 i = 0; i < organizationIdCounter.current(); i++) {
            ...
            if (org.organizationAddress == msg.sender) {
                return (i+1);
            }
        }
        return(0);
